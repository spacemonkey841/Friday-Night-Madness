...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			case 'chicken-dance':
				dialogue = CoolUtil.coolTextFile(Paths.txt('chicken-dance/Dialogue0'));
			case 'grunting':
				dialogue = CoolUtil.coolTextFile(Paths.txt('grunting/Dialogue1'));
			case 'redeemer':
				dialogue = CoolUtil.coolTextFile(Paths.txt('redeemer/Dialogue2'));
			case 'gunshots':
				dialogue = CoolUtil.coolTextFile(Paths.txt('gunshots/Dialogue3'));
			case 'senpai':
		...
		switch(stageCheck)
		{
			...
			case 'mall':
			{
					...
					var bg:FlxSprite = new FlxSprite(-500, -500).loadGraphic(Paths.image('christmas/bgEscalator','week5'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.2, 0.2);
					bg.active = false;
					bg.updateHitbox();
					...
					upperBoppers = new FlxSprite(-600, -450);
					upperBoppers.frames = Paths.getSparrowAtlas('christmas/bottomBop','week5');
					upperBoppers.animation.addByPrefix('bop', "Grunts Bop", 24, false);
					...
					var tree:FlxSprite = new FlxSprite(370, -275).loadGraphic(Paths.image('christmas/christmasTree','week5'));
					tree.antialiasing = true;
					tree.scrollFactor.set(0.40, 0.40);
					add(tree);

					bottomBoppers = new FlxSprite(-600, -400);
					bottomBoppers.frames = Paths.getSparrowAtlas('christmas/upperBop','week5');
					bottomBoppers.animation.addByPrefix('bop', 'Grunts Bop', 24, false);
			...
			case 'stage':
				{
						defaultCamZoom = 0.6;
						curStage = 'stage';
						var bg:FlxSprite = new FlxSprite(-1050, -500).loadGraphic(Paths.image('stageback'));
						bg.antialiasing = true;
						...
						var stageCurtains:FlxSprite = new FlxSprite(-600, -500).loadGraphic(Paths.image('stagecurtains'));
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (SONG.song.toLowerCase() == 'gunshots')
					{
	...
	override function stepHit()
	{
		...
		if (curSong == 'deez') 
		{
			switch (curStep)
			{
				...
				case 1188:
					remove(dad);
					dad = new Character(100, 100, 'gurt');
					add(dad);
 			}
		}
		else if (curSong == 'gunshots')
		{
			switch (curStep)
			{
				case 48:
				dad.playAnim('gruntShoot', true);
				FlxG.camera.shake(0.05, 0.05);
				case 49:
				boyfriend.playAnim('dodge', true);
				case 320:
